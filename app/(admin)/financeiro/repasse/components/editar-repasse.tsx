...
import { AlertCircle, Check, LoaderCircle, Plus, Trash2, X } from "lucide-react";
import { calculateRepasse } from "@/lib/financeiro/repasse-calculo";
import type { RepasseItem, RepasseOtherDeduction, RepasseUpdateInput } from "@/lib/financeiro/repasse-types";
...
export default function EditarRepasse({ item, onClose, onSaved }: EditarRepasseProps) {
  ...
  const calculation = useMemo(() => calculateRepasse({
    ...
    adminFeePercent,
    deductionValues: item.deductions.filter((deduction) => selectedIds.includes(deduction.id)).map((deduction) => deduction.value),
    otherDeductionValues: otherDeductions.map((deduction) => deduction.value),
  }), [adminFeePercent, item, otherDeductions, selectedIds]);
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-[#280003]/45 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-labelledby="editar-repasse-title">
      ...
      <aside className="relative flex h-full w-full max-w-2xl flex-col overflow-hidden bg-[#F7F7F9] shadow-2xl">
        <header className="flex items-start justify-between border-b border-gray-200 bg-white px-6 py-5">
          ...
          <button type="button" onClick={onClose} disabled={saving} className="rounded-xl p-2 text-gray-400 transition hover:bg-gray-100 hover:text-gray-700"><X className="h-5 w-5" /></button>
        </header>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-5 sm:p-6">
          ...
          <section className="rounded-3xl bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-black text-[#280003]">Manutenções e despesas</h3><p className="mt-0.5 text-xs text-gray-400">Marque os valores que serão descontados deste repasse.</p></div></div>
          ...
          <section className="rounded-3xl bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-black text-[#280003]">Outros descontos</h3><p className="mt-0.5 text-xs text-gray-400">Ex.: IPTU, condomínio, limpeza, troca de registro.</p></div><button type="button" onClick={addOtherDeduction} className="inline-flex min-h-9 items-center gap-1 rounded-xl bg-[#004777]/10 px-3 text-xs font-bold text-[#004777] hover:bg-[#004777]/15"><Plus className="h-3.5 w-3.5" />Adicionar</button></div>
            <div className="mt-4 space-y-2">
              ...
              {otherDeductions.map((deduction) => (
                <div key={deduction.id} className="grid grid-cols-[1fr_130px_36px] gap-2">
                  ...
                  <button type="button" aria-label="Excluir desconto" onClick={() => setOtherDeductions((current) => current.filter((item) => item.id !== deduction.id))} className="rounded-xl text-gray-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="mx-auto h-4 w-4" /></button>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-3xl bg-[#004777] p-5 text-white shadow-lg shadow-[#004777]/15">
            <div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-wider text-white/60">Valor líquido a repassar</p><strong className="mt-1 block text-3xl">{currency(calculation.netValue)}</strong></div><div className="text-right text-xs leading-relaxed text-white/70">Bruto {currency(calculation.grossValue)}<br />Descontos − {currency(calculation.adminFeeValue + calculation.deductionTotal)}</div></div>
        ...
        <footer className="flex items-center justify-end gap-3 border-t border-gray-200 bg-white px-6 py-4">
